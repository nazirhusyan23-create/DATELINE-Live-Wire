...
1. GitHub par ek naya empty repository banayein (e.g. `dateline-news-app`) — README/`.gitignore` add na karein.
2. Apne computer par is folder mein terminal kholein aur yeh commands chalayein:
...
1. [vercel.com](https://vercel.com) par jaakar GitHub account se sign in karein.
2. **"Add New… → Project"** click karein.
3. Apni `dateline-news-app` repository select karein aur **Import** karein.
4. Framework preset **"Other"** rehne dein — koi build command ki zarurat nahi, "Build Command" aur "Output Directory" fields khali/default hi rehne dein. Vercel `api/news.js` ko automatically ek serverless function ke tor par detect kar legi.
5. **Deploy** dabayein. 30-60 seconds mein aapki app live ho jayegi — news, weather, sab kaam karega — Vercel ek `.vercel.app` URL de dega.
...
- **ipapi.co** aur **bigdatacloud.net** — free location fallback/reverse-geocoding
